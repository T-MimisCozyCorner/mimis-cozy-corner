...
if(p.id==="twin-story"){
 ...
 <div style={{minHeight:"100vh",background:"#faf8f3",paddingTop:80}}>
   <div style={{position:"fixed",top:0,left:0,right:0,background:"#fff",borderBottom:"1px solid #e2dccc",padding:12,display:"flex",justifyContent:"space-between",zIndex:10}}>
     <b>Twin Story Method - Original Workbook</b><button onClick={onBack}>Back</button>
   </div>
   <div style={{maxWidth:900,margin:"0 auto",padding:20}}>
     <iframe src="/pdfs/twin-story-method-workbook.pdf" style={{width:"100%",height:"80vh",border:"1px solid #d4af37",borderRadius:16}} />
     <div style={{marginTop:16,background:"#1a1a2e",color:"#fff",padding:16,borderRadius:12}}>
       <h3 style={{color:"#d4af37"}}>Original 50-Day Workbook</h3>
       <p>Finding Faith journal - your uploaded original</p>
       <a href="/pdfs/twin-story-method-workbook.pdf" target="_blank" style={{color:"#d4af37"}}>Download PDF - 50 Day Journal</a>
     </div>
   </div>
 </div>
 )
...
if(p.id==="creative"){
 ...
 lines = [p.desc, `Content for page ${safePg} of ${total}`];
}
...
<div style={{marginTop:24,padding:16,background:"#1a1a2e",borderRadius:12,color:"#fff"}}><h3 style={{color:"#d4af37",marginTop:0}}>Workbook PDF</h3>{p.pdfs.map((pdf:string)=><p key={pdf}><a href={`/pdfs/${pdf}`} target="_blank" style={{color:"#d4af37"}}>{pdf} - Open Workbook</a></p>)}<button onClick={onBack} style={{marginTop:12,background:"#d4af37",padding:"10px 20px",borderRadius:8,border:"none",fontWeight:700}}>Back to Library</button></div>
</div></div>
...
<div style={{background:"white",borderBottom:"3px solid #ff00ff",padding:10,textAlign:"center"}}><h1 style={{fontSize:24,fontWeight:900,margin:0}}>mimiscozycorner.com</h1><p style={{fontSize:10,color:"#999",margin:0}}>Clean - Original Workbook - No 4 Brands line</p></div>
