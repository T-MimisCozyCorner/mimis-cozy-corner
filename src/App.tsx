...
<div style={{padding:"16px",background:p.id==="shop"?"#fff0ff":"#fef2f8"}}><div style={{display:"flex",justifyContent:"space-between"}}><div style={{fontWeight:900,fontSize:"14px",color:"#a2006d"}}>{p.title}</div><span style={{fontSize:"9px",background:p.color,color:"white",padding:"4px 10px",borderRadius:"12px"}}>{p.badge}</span></div><div style={{fontSize:"11px",color:"#666",marginTop:"6px"}}>{p.desc}</div><button onClick={()=>{ if(p.link){ window.open(p.link,"_blank"); } else { window.location.hash=`#product-${p.id}`; setHash(`#product-${p.id}`); } }} style={{marginTop:"12px",width:"100%",background:p.color,color:"white",border:"none",padding:"12px",borderRadius:"10px",fontWeight:800,cursor:"pointer"}}>{p.id==="shop"?"SHOP NOW 330 DESIGNS →":"OPEN WORKBOOK →"}</button></div>
</div>)}
</div>
</div>
);
